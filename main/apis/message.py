...
def send_message(recipient_id):
    ...
    if not sender or not recipient:
        error_response(404, 'Sender or recipient not found')
    ...
    return success_response(201, 'success', 'Message sent successfully')
...
def delete_message(message_id):
    ...
    if message.sender_id != current_user_id and message.recipient_id != current_user_id:
        error_response(403, 'You are not authorized to delete this message')

    db.session.delete(message)
    db.session.commit()
    return success_response(200, 'success','Message deleted successfully')
